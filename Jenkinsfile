pipeline {
    ...
    parameters {
        ...
        booleanPararm('name': 'my_boolean', defaultValue: false, descrition: 'this is boolean param')
        choice('name': 'my_choice', choices: ['money', 'game'], description: 'this is choices param')
        password('name': 'my_password', defaultValue: 'mypass', descrition: 'this is password pararm')
    }
    stages {
        stage("build"){
            when {
